Bela Core version 0.14 is now available from:

  <http://github.com/tjm2013/bela-new/>
...
updates will be released anymore. Without security updates, using a Bela
...
ECDSA signatures inside Bela transactions now use validation using
...
Previous versions of Bela Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Bela Core was to raise the default minimum
relay fee.

Bela Core 0.14 will have a strict maximum size on the mempool. The
...
Bela Core 0.14 also introduces new default policy limits on the length and
...
Bela Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Bela Core will relay transactions with
...
entirely in Bela Core 0.14, and it is at this time undecided whether the
...
Bela Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Bela Core automatically creates a hidden service to listen on, without
manual configuration. Bela Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Bela Core is listening, and
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Bela
Core to use floating fees. By default, Bela Core will use floating
fees.
...
At all times, Bela Core will cap fees at `-maxtxfee=<x>` (default:
0.10) Bela.
Furthermore, Bela Core will never create transactions paying less than
...
Various code modernizations have been done. The Bela Core code base has
...
for a new opcode in the Bela scripting system that in combination with
...
Bela transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Bela Core.
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Bela wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Bela block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of Bela they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of Bela being spent is exactly
...
- **Increased security for multisig:** Bela addresses (both P2PKH addresses
  ...
  believed to be provided by Bela's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Bela's current ledger (called the UTXO set),
  ...
  note that Bela Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Bela users to individually opt-in to almost any change in the Bela
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Bela Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Bela
client to Bela Core 0.14 to signal for segwit support. When a
...
Since Bela Core 0.13.1, nodes have defaulted to only relaying and
